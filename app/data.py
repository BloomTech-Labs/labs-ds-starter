...
class Data:
    ...
    def seed(self, n_rows: int):
        ...
        def random_row():
            features = tuple(randint(1, 10) for _ in range(n_features))
            return *features, sum(features)

        def joined_rows():
            return ",".join(str(random_row()) for _ in range(n_rows))
    ...
    def insert(self, feature_1, feature_2, feature_3, target):
        ...
        VALUES ({},{},{},{});""").format(
            ...
            sql.Literal(self.cleaner(target)),
        ))
...
if __name__ == '__main__':
    ...
    # db.reset()
    # db.seed(2048)
    # print(db.row(db.insert(0, 0, 0, "0")))
    print(db.df())
